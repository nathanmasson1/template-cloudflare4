...
export default function ConfigEditor() {
    ...
    useEffect(() => {
        cmsApi('read', 'src/data/siteConfig.json')
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (pendingLogo) {
                ...
                const uploadPath = `public/uploads/${Date.now()}-logo.${fileExt}`;
                await cmsApi('write', uploadPath, { content: base64Content, isBase64: true, message: 'CMS: Upload Logo' });
                configCopy.logo = uploadPath.replace('public', '');
            }
            if (pendingFooterLogo) {
                ...
                const uploadPath = `public/uploads/${Date.now()}-footer-logo.${fileExt}`;
                await cmsApi('write', uploadPath, { content: base64Content, isBase64: true, message: 'CMS: Upload Footer Logo' });
                configCopy.footerLogo = uploadPath.replace('public', '');
            }
            if (pendingFavicon) {
                ...
                const uploadPath = `public/uploads/${Date.now()}-favicon.${fileExt}`;
                await cmsApi('write', uploadPath, { content: base64Content, isBase64: true, message: 'CMS: Upload Favicon' });
                configCopy.favicon = uploadPath.replace('public', '');
            }
            const res = await cmsApi('write', 'src/data/siteConfig.json', { content: JSON.stringify(configCopy, null, 2), sha: fileSha, message: 'CMS: Update siteConfig.json' });
    ...
    if (loading) return (
        <div className="flex flex-col items-center justify-center p-20 text-slate-400 bg-white rounded-3xl border border-slate-200">
            ...
            <p className="font-medium animate-pulse">Carregando configuracoes...</p>
        </div>
    );
        ...
        <form onSubmit={handleSave} className="space-y-8 pb-32 max-w-3xl">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div className="md:col-span-2 space-y-6">
                        ...
                        <div>
                            ...
                            <textarea rows={2} placeholder="Breve descrição institucional usada como fallback de SEO e rodapé" value={config?.description || ''} onChange={e => setConfig({ ...config, description: e.target.value })} className={`${inputClass} resize-y`} />
                        </div>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            <div>
                                <label className={labelClass}>Texto da Barra Superior</label>
                                <input type="text" placeholder="Agenda e negocios" value={config?.navbarTopText || ''} onChange={e => setConfig({ ...config, navbarTopText: e.target.value })} className={inputClass} />
                            </div>
                            <div>
                                <label className={labelClass}>Texto do Botao Principal</label>
                                <input type="text" placeholder="Divulgar agora" value={config?.ctaText || ''} onChange={e => setConfig({ ...config, ctaText: e.target.value })} className={inputClass} />
                            </div>
                        </div>
                        {/* Preset Themes */}
                        <div>
            ...
            <div className="p-8 bg-white border border-slate-200 rounded-2xl shadow-sm">
                ...
                <div className="space-y-4">
                    ...
                    <div><label className={labelClass}>Texto de Copyright</label><input type="text" placeholder="© {year} {siteName}. Todos os direitos reservados." value={config?.footer?.copyright || ''} onChange={e => setConfig({ ...config, footer: { ...config.footer, copyright: e.target.value } })} className={inputClass} /></div>
                </div>
            </div>
            ...
            <div id="ai" className="p-8 bg-white border border-slate-200 rounded-2xl shadow-sm scroll-mt-8">
                ...
                <div className="space-y-4">
                    <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-xs text-amber-800">
                        <strong>⚠️ Atenção:</strong> A chave fica salva em <code className="bg-amber-100 px-1 rounded">siteConfig.json</code> (armazenada no D1 em producao). Para mais segurança, defina <code className="bg-amber-100 px-1 rounded">OPENAI_API_KEY</code> como variável de ambiente na Cloudflare — ela tem precedência sobre este campo.
                    </div>
                    <div>
